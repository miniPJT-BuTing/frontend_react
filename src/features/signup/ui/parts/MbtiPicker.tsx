...
export default function MbtiPicker() {
  ...
  return (
    <div className="flex flex-col gap-4">
      <label className="text-lg font-bold">MBTI</label>
      {/* Outer row: 4 Columns */}
          ...
          <div key={index} className="flex flex-col gap-3 flex-1">
            {pair.map((char) => (
              <RetroButton
                ...
                className={`w-full aspect-square !rounded-xl text-xl font-bold flex items-center justify-center ${mbti?.[index] === char ? '!bg-[#F7ABCF]' : ''}`}
